...
c	ddphi(i,j) has [d^2(phi_i)/dx^2]|x=x_j
	real*8 wt,x
...
c		write(6,*)'x(j)-x(i)=',x(j)-x(i),'legi=',legi
	ddphi(i,j)=(-2.d0*dphi(i,j)+dx0(lmax+1)/legi)/(x(j)-x(i))
	endif
	if(i.eq.j) then
	y=x(j)
	call legend(y,x0,dx0)
	dphi(i,j)=0.5d0*dx0(lmax+1)/x0(lmax+1)
c	Next line is the second derivative hardwired for lmax+1=5, i.e., (d^2/dx^2) P_5(x)
	ddx0=(1260.d0*y**3-420.d0*y)/8.d0
	ddphi(i,j)=(1.d0/3.d0)*ddx0/x0(lmax+1)
	endif
	enddo
	enddo
...
c	dphixx(k,l) = (d^2/d(x1^2)) [ phi_i1 (x1) * phi_i2 (x2) ] | x1=x_j1,x2=x_j2
	dphixx(k,l)=ddphi(i1,j1)*phi(i2,j2)
c	dphixz(k,l) = (d^2/[d(x1) d(x2)]) [ phi_i1 (x1) * phi_i2 (x2) ] | x1=x_j1,x2=x_j2
	dphixz(k,l)=dphi(i1,j1)*dphi(i2,j2)
c	dphizz(k,l) = (d^2/d(x2^2)) [ phi_i1 (x1) * phi_i2 (x2) ] | x1=x_j1,x2=x_j2
	dphizz(k,l)=phi(i1,j1)*ddphi(i2,j2)
	enddo
	enddo
	enddo
	enddo
